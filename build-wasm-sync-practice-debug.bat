@echo off
echo Building Defcon WebAssembly Debug (Silo Practice Client)...

set ORIGINAL_DIR=%CD%
call "%ORIGINAL_DIR%\contrib\systemIV\contrib\emsdk\emsdk_env.bat" > nul 2>&1
set NINJA_PATH=%ORIGINAL_DIR%\tools\ninja.exe
if not exist build\wasm-sync_practice-debug mkdir build\wasm-sync_practice-debug
cd /d %ORIGINAL_DIR%\build\wasm-sync_practice-debug

echo Configuring...
python "%ORIGINAL_DIR%\contrib\systemIV\contrib\emsdk\upstream\emscripten\emcmake.py" cmake ^
    -DCMAKE_BUILD_TYPE=Debug ^
    -DCMAKE_CXX_FLAGS_DEBUG="-g -O0 -D_DEBUG -DSYNC_PRACTICE=1 -DNOT_REPLAY_VIEWER=1" ^
    -DCMAKE_C_FLAGS_DEBUG="-g -O0 -D_DEBUG -DSYNC_PRACTICE=1 -DNOT_REPLAY_VIEWER=1" ^
...
echo Output: %ORIGINAL_DIR%\build\wasm-sync_practice-debug\result\Debug\
...
copy /Y "%ORIGINAL_DIR%\build\wasm-sync_practice-debug\result\Debug\sync_practice_*.js" "%ORIGINAL_DIR%\website\sync_practice\" 2>nul
copy /Y "%ORIGINAL_DIR%\build\wasm-sync_practice-debug\result\Debug\sync_practice_*.wasm" "%ORIGINAL_DIR%\website\sync_practice\" 2>nul
copy /Y "%ORIGINAL_DIR%\build\wasm-sync_practice-debug\result\Debug\sync_practice_*.data" "%ORIGINAL_DIR%\website\sync_practice\" 2>nul
