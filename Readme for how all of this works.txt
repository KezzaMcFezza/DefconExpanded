...
1. Be very careful when modifying the python scripts its only 1 line of code away from playing a game and then wondering why the log processor window didn't generate a log file
2. You can check the MySQL database and you will see how the player data is inserted into it
3. When deleting a demo from the database make sure to delete the demo file from the local machine also or it will add it to the pending list and remain there for ever and ever
4. The log files are temperamental you can experiment like I have by making your own fake games and dcrec files to go along with them but be careful as sometimes the server.js
   likes to shit itself when it cant parse the json data if you make a mistake lol
5. And to be honest if you are still confused about how all this works just message me ive been dreaming in MySQL and command prompts past few days so I most likely have had
   any issues that you are having if they appear
