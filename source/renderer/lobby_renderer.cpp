...
void LobbyRenderer::RenderOverlay()
{
    ...
    while( true )
    {
        ...
        const char *string = LANGUAGEPHRASE(stringId);
...
void LobbyRenderer::RenderVersionInfo()
{
    ...
    char currentVersion[256] = "BETA" "  " "1.16";
#elif defined(REPLAY_VIEWER) || defined(REPLAY_VIEWER_DESKTOP)
    char currentVersion[256] = "RELEASE" "  " "1.25.3";
#else
...
void LobbyRenderer::RenderAuthStatus()
{
    ...
    {
        strcpy( authStatusString, LANGUAGEPHRASE("dialog_auth_status_demo_user") );
    }
...
void LobbyRenderer::RenderTestHours()
{
    ...
    if( success )
    {
        ...
        const char *amPm = (timeHours >= 12 ? LANGUAGEPHRASE("dialog_time_pm") : LANGUAGEPHRASE("dialog_time_am") );
...
void LobbyRenderer::RenderNetworkIdentity()
{
    ...
    Authentication_GetKey( authKey );
    int authStatus = Authentication_GetStatus(authKey);
    const char *authStatusString = LANGUAGEPHRASE(Authentication_GetStatusStringLanguagePhrase(authStatus));

    char caption[256];
    strcpy( caption, LANGUAGEPHRASE("dialog_lobby_authentication") );
    ...
    if( g_app->GetClientToServer() )
    {
        ...
        if( identityKnown )
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_lobby_client_ident") );
        ...
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_lobby_client_ident_unknown") );
    ...
    if( g_app->GetServer() )
    {
        ...
        if( identityKnown )
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_lobby_server_ident") );
			LPREPLACESTRINGFLAG( 'I', publicIP, caption );
			LPREPLACEINTEGERFLAG( 'P', publicPort, caption );
            col.Set(0,255,0,255);
        }
        ...
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_lobby_server_ident_unknown") );
            col.Set(200,200,30,255);
        }
