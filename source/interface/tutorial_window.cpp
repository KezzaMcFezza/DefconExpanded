...
void TutorialWindow::Render( bool _hasFocus )
{
    ...
    if( g_app->GetTutorial() )
    {
        ...
        const char *title = g_app->GetTutorial()->GetCurrentLevelName();
        char fullTitle[512];
        strcpy( fullTitle, LANGUAGEPHRASE("tutorial_chapter_desc") );
        ...
        const char *msg = g_app->GetTutorial()->GetCurrentMessage();
        if( msg )
        {
...
void TutorialChapterWindow::Create()
{
    ...
    for( int i = 1; i <= 7; ++i )
    {
		...
		char caption[128];
        strcpy( caption, LANGUAGEPHRASE("tutorial_level") );
