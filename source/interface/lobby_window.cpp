    ...
    void Render( int realX, int realY, bool highlighted, bool clicked )
    {
        ...
        else
        {
            ...
            else
            {
				...
				{
                    strcpy( teamNameFull, LANGUAGEPHRASE("dialog_demo_team_name") );
					LPREPLACESTRINGFLAG( 'T', teamName, teamNameFull );
				}
                else
				{
                    ...
                    g_app->GetServer()->m_teams.ValidIndex(team->m_teamId) )
                {
                    ...
                    if( sToC )
                    {
                        ...
                        strcpy( netLocation, LANGUAGEPHRASE("tooltip_lobby_address_port") );
        ...
        if( m_teamIndex >= maxTeams )
        {
            ...
            {                
                    ...
                    m_teamIndex >= maxDemoSize )
                {
                    ...
                    strcpy( badKey->m_extraMessage, LANGUAGEPHRASE("dialog_demo_too_many_players") );
...
void LobbyOptionsWindow::Render( bool _hasFocus )
{
    ...
    const char *currentButton = EclGetCurrentButton();
...
void LobbyWindow::Update()
{
    ...
    if( authResult < 0 )
    {
        ...
        char msg[2048];
        strcpy( msg, LANGUAGEPHRASE("dialog_not_permitted_start_game") );
...
void LobbyWindow::Render( bool _hasFocus )
{
        ...
        else
        {
            ...
            {            
                if( g_app->GetWorld()->m_spectators.ValidIndex(i) )
                {
                    ...
                    {
                        strcpy( specName, LANGUAGEPHRASE("dialog_demo_team_name") );
    ...
    {
        char caption[256];
        strcpy( caption, LANGUAGEPHRASE("dialog_gamestart") );
...
:   InterfaceWindow( "RenameTeamWindow" )
{
	...
	{
		m_teamId = -1;
		const char *prefPlayerName = g_preferences->GetString( "PlayerName" );
