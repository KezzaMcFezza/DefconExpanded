...
{    
    ...
    for( int i = 0; i <= g_app->GetWorld()->m_chat.Size(); i++ )
    {
        ...
        if( chatMsg && chatMsg->m_visible )
        {
            ...
            {
                action = true;
                snprintf( newMsg, sizeof(newMsg), "* %s %s", chatMsg->m_playerName, chatMsg->m_message.c_str() );
            }
            else
            {
            ...
            {
                action = true;
                snprintf( newMsg, sizeof(newMsg), "* %s %s", chatMsg->m_playerName, chatMsg->m_message.c_str()+4 );
                newMsg[ sizeof(newMsg) - 1 ] = '\0';
            }
