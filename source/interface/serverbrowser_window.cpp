...
public:
    void Render( int realX, int realY, bool highlighted, bool clicked )
    {
        ...
        else
        {
            ...
            char caption[256];
            strcpy( caption, LANGUAGEPHRASE("dialog_refresh_number_server") );
...
public:
    ...
    void Render( int realX, int realY, bool highlighted, bool clicked )
    {
            ...
            sbw->m_serverList->ValidIndex( realIndex ) )
        {
            ...
            {
                strcpy( netLocation, LANGUAGEPHRASE("dialog_net_location_lan") );
            ...
            if( server->HasData( NET_METASERVER_GAMEMODE, DIRECTORY_TYPE_CHAR ) )
            {
                ...
                if( gameMode > 0 )
                {
                    ...
                    {
                        strcpy( gameModeString, GameOption::TranslateValue( option->m_subOptions[gameMode] ) );
                    }
                }
            }
            ...
            if( isDemoServer )
            {
                strcpy( gameModeString, LANGUAGEPHRASE("dialog_demo_game_mode") );
            }
            ...
            if( server->HasData( NET_METASERVER_MODPATH ) )
            {
                ...
                if( tokens->ValidIndex(0) )
                {
                    ...
                    if( numRemaining > 0 )
                    {
                        strcpy( modPath, LANGUAGEPHRASE("dialog_mod_base_number_more") );
            ...
            if( !protocolMatch )
            {
                strcpy( serverNameFull, LANGUAGEPHRASE("dialog_server_name_incompatible") );
				LPREPLACESTRINGFLAG( 'S', serverName, serverNameFull );
                col.Set(255,30,30,155);
            }
            else if( !modPathOk )
            {
                strcpy( serverNameFull, LANGUAGEPHRASE("dialog_server_name_mods_required") );
				LPREPLACESTRINGFLAG( 'S', serverName, serverNameFull );
                col.Set(255,255,50,155);
            }
            else if( ourServer )
            {
                strcpy( serverNameFull, LANGUAGEPHRASE("dialog_server_name_your_server") );
				LPREPLACESTRINGFLAG( 'S', serverName, serverNameFull );
                col.Set(0,255,0,155);
            }
            else if( demoRestricted )
            {
                strcpy( serverNameFull, LANGUAGEPHRASE("dialog_server_name_demo_restricted") );
				LPREPLACESTRINGFLAG( 'S', serverName, serverNameFull );
            ...
            else
            {
                ...
                if( (numTeams >= maxTeams || inProgress) &&
                    numSpectators >= maxSpectators )
                {
                    strcpy( serverNameFull, LANGUAGEPHRASE("dialog_server_name_full") );
					LPREPLACESTRINGFLAG( 'S', serverName, serverNameFull );
                    col.Set( 100,100,100,150);
                }
            }            
...
public:
    ...
    {      
        ...
        if( parent->m_sortType == m_sortType )
        {
            // Convert deprecated glColor4ub and GL_TRIANGLES to modern renderer
            Colour arrowColor(200, 200, 255, 200);
...
bool ServerBrowserWindow::ConnectToServer( Directory *_server, const char *_serverPassword )
{
    ...
    if( !protocolMatch )
    {
        ...
        char msg[1024];
        strcpy( msg, LANGUAGEPHRASE("dialog_server_version_mismatch") );
    ...
    {
        char msg[1024];
        strcpy( msg, LANGUAGEPHRASE("dialog_no_join_own_game") );
...
{
    if( !g_app->GetServer() ) return false;
...
void ServerBrowserWindow::FilterServerList()
{
    ...
    if( m_listType == ListTypeRecent )
    {
        for( int i = 0; i < g_gameHistory.m_servers.Size(); ++i )
        {
            ...
            char copiedId[512];
            strcpy( copiedId, server->m_identity );
            ...
            if( colon )
            {
                ...
                *colon = '\x0';
                ...
                if( !IsServerInList( ip, port ) )
                {
                    ...
                    char fullName[512];
                    strcpy( fullName, LANGUAGEPHRASE("dialog_server_name_not_responding") );
...
{
    char name[512];
    strcpy( name, _server->GetDataString( NET_METASERVER_SERVERNAME ) );
    strlwr( name );
...
int ServerBrowserWindow::ScoreServer_Mods( Directory *_server )
{
    ...
    {
        char name[512];
        strcpy( name, _server->GetDataString( NET_METASERVER_MODPATH ) );
        strlwr( name );
...
void ServerBrowserWindow::Update ()
{
    ...
    if( authResult < 0 )
    {
        ...
        const char *errorString = LANGUAGEPHRASE(Authentication_GetStatusStringLanguagePhrase(authResult));
        ...
        strcpy( window->m_extraMessage, LANGUAGEPHRASE("dialog_not_permitted_join_game") );
