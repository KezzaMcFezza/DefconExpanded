...
FadingWindow::FadingWindow( const char *_name )
...
void FadingWindow::SaveProperties( const char *_name )
{
    char s_result[256];
...
void FadingWindow::LoadProperties( const char *_name )
{
    const char *theString = g_preferences->GetString( _name );
    if( theString )
    {
        int x, y, w, h;
