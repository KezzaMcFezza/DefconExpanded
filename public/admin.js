...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('upload-macos-arm64-dedcon').addEventListener('submit', uploadDedconBuild);
    document.getElementById('edit-dedcon-resource-form').addEventListener('submit', saveEditDedconBuild);
    document.getElementById('cancel-edit').addEventListener('click', cancelEditDedcon);
});
...
function initializeAdminPanel() {
    ...
    if (document.getElementById('player-search-btn')) {
...
const serverConfigs = [
    {
        name: "DefconExpanded | 1v1 | Totally Random",
    ...
    {
        name: "DefconExpanded | 1v1 | Best Setups 2",
        filename: "1v1configbest2.txt"
    },
    {
    ...
    },
    {
        ...
        filename: "1v1configtest.txt"
    },
    {
        name: "New Player Server",
        filename: "noobfriendly.txt"
    },
    {
        name: "DefconExpanded | 2v2 | Totally Random",
    ...
    {
        name: "DefconExpanded | Free For All | Random Cities",
    ...
    {
        name: "DefconExpanded | 8 Player | Diplomacy",
        filename: "8playerdiplo.txt"
    },
    {
        name: "DefconExpanded | 10 Player | Diplomacy",
        filename: "10playerdiplo.txt"
    },
    {
...
async function loadFileContent(filename) {
    try {
        ...
        await alert(`Error loading file: ${error.message}`);
    }
}
...
function editDemo(demoId) {
    ...
    try {
        ...
        alert('Error loading demo editor: ' + error.message);
    }
}
...
async function saveEditDemo(event) {
    ...
    try {
        ...
        await confirm('Error parsing game data');
        return;
    }
...
async function loadDedconBuilds() {
    try {
        if (window.location.pathname.includes('/dedconmanagment')) {
            const response = await fetch('/api/admin/dedcon-builds');
...
async function uploadDedconBuild(event) {
    event.preventDefault();
    if (currentUserRole > 2) {
        alert('You do not have permission to upload builds');
        return;
    }
    ...
    try {
        const response = await fetch('/api/upload-dedcon-build', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            const result = await response.json();
            alert(`${platform.charAt(0).toUpperCase() + platform.slice(1)} build uploaded successfully`);
            loadDedconBuilds();
            event.target.reset();
        } else {
            const errorData = await response.json();
            alert('Failed to upload build: ' + (errorData.error || 'Unknown error'));
        }
    } catch (error) {
        console.error('Error uploading build:', error);
        alert('Error uploading build: ' + error.message);
    }
}

async function editDedconBuild(buildId) {
    if (currentUserRole > 2) {
        alert('You do not have permission to edit builds');
        return;
    }
    try {
        ...
        alert('Error fetching build details: ' + error.message);
    }
}
...
async function saveEditDedconBuild(event) {
    event.preventDefault();
    if (currentUserRole > 2) {
        alert('You do not have permission to save build edits');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            const error = await response.json();
            alert('Failed to update build: ' + (error.error || 'Unknown error'));
        }
    } catch (error) {
...
async function deleteDedconBuild(buildId) {
    if (currentUserRole > 2) {
        await alert('You do not have permission to delete builds');
        return;
    }
    const confirmed = await confirm('Are you sure you want to delete this build?');
    if (confirmed) {
        try {
            const response = await fetch(`/api/dedcon-build/${buildId}`, {
                method: 'DELETE'
            ...
            if (response.ok) {
                await alert('Build deleted successfully');
                loadDedconBuilds();
            } else {
                await alert('Failed to delete build');
            }
        } catch (error) {
            console.error('Error deleting build:', error);
            await alert('Error deleting build: ' + error.message);
        }
    }
}

function cancelEditDedcon() {
    document.getElementById('dedcon-build-edit').style.display = 'none';
