		...
		if ( strcmp( mouseDownWindow, "None" ) != 0 )
		{
			...
			else
			{
					 ...
					 mouseDownWindowX >= 0.0f )
				{
					...
					if ( sizeChanged )
					{
						if ( newWidth < window->m_minW )
							newWidth = window->m_minW;
						if ( newHeight < window->m_minH )
							newHeight = window->m_minH;
...
int EclMouseStatus()
{
	...
	if ( resizeH &&
		 resizeV )
		return EclMouseStatus_ResizeHV;
	if ( resizeH )
		return EclMouseStatus_ResizeH;
	if ( resizeV )
		return EclMouseStatus_ResizeV;
...
void EclRender()
{
	...
	if ( strcmp( maximisedWindow, "None" ) != 0 )
	{
		...
		if ( maximised )
		{
			...
			g_renderer2d->BeginTriangleFillBatch();
			...
			g_renderer2d->EndTriangleFillBatch();
...
void EclRegisterWindow( EclWindow *window, EclWindow *parent )
{
	...
	if ( parent && window->m_x == 0 && window->m_y == 0 )
	{
		...
		if ( left > window->m_w / 2 )
			window->m_x = int( parent->m_x + parent->m_w * (float)rand() / (float)RAND_MAX );
		else
			window->m_x = int( parent->m_x - window->m_w * (float)rand() / (float)RAND_MAX );
		if ( above > window->m_h / 2 )
			window->m_y = int( parent->m_y + parent->m_h * (float)rand() / (float)RAND_MAX );
		else
			window->m_y = int( parent->m_y - window->m_h / 2 * (float)rand() / (float)RAND_MAX );
	}
	...
	if ( window->m_y < 0 )
		window->m_y = 0;
	if ( window->m_x + window->m_w > screenW )
		window->m_x = screenW - window->m_w;
	if ( window->m_y + window->m_h > screenH )
		window->m_y = screenH - window->m_h;
