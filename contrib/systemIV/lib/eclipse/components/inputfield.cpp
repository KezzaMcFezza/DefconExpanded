...
void InputField::Render( int realX, int realY, bool highlighted, bool clicked )
{
	...
	if ( m_align == -1 )
		fieldX = realX + 5;
	if ( m_align == 0 )
		fieldX = realX + m_w / 2 - textWidth / 2;
	if ( m_align == 1 || tooWide )
		fieldX = realX + m_w - textWidth - 5;
...
void InputField::Keypress( int keyCode, bool shift, bool ctrl, bool alt, unsigned char ascii )
{
	...
	if ( ascii == '%' )
		return; // This is a banned character because it breaks
