...
InterfaceWindow::InterfaceWindow( const char *name, const char *title, bool titleIsLanguagePhrase )
...
void InterfaceWindow::OnResize( int newWidth, int newHeight, int oldWidth, int oldHeight )
{
	...
	if ( m_x + m_w > newWidth )
		m_x = std::max( 0, newWidth - m_w );
	if ( m_y + m_h > newHeight )
		m_y = std::max( 0, newHeight - m_h );
	if ( m_x < 0 )
		m_x = 0;
	if ( m_y < 0 )
		m_y = 0;
...
void InterfaceWindow::CreateValueControl( const char *name, int x, int y, int w, int h,
										  ...
										  InterfaceButton *callback, const char *tooltip, bool tooltipIsLanguagePhrase,
...
void InterfaceWindow::Render( bool hasFocus )
{
	...
	{
		snprintf( caption, sizeof( caption ), "%s", LANGUAGEPHRASE( m_title ) );
	}
	else
	{
		snprintf( caption, sizeof( caption ), "%s", m_title );
	}
	caption[sizeof( caption ) - 1] = '\0';

	if ( titleFont->m_uppercase )
		strupr( caption );
