...
void DirectoryData::WriteXML( ostream &o, int indent )
{
	...
	switch ( m_type )
	{
		...
		case DIRECTORY_TYPE_STRING:
			WriteXMLString( o, m_string.c_str() );
			break;
...
#elif defined( FIXED64_NUMERICS )
		case DIRECTORY_TYPE_FIXED:
			AppDebugOut( "%f\n", Fixed::Fixed( m_fixed ).DoubleValue() );
			break;
#endif
		case DIRECTORY_TYPE_CHAR:
			AppDebugOut( "(val=%d)\n", (int)m_char );
...
bool DirectoryData::Read( istream &input )
{
	...
	{
		case DIRECTORY_TYPE_INT:
			readNetworkValue( input, m_int );
			break;
		case DIRECTORY_TYPE_FLOAT:
			readNetworkValue( input, m_float );
			break;
		case DIRECTORY_TYPE_FIXED:
			readNetworkValue( input, m_fixed );
			break;
		case DIRECTORY_TYPE_CHAR:
			input.read( (char *)&m_char, sizeof( m_char ) );
			break;
		case DIRECTORY_TYPE_BOOL:
			m_bool = input.get();
			break;
...
void DirectoryData::Write( ostream &output ) const
{
	...
	{
		case DIRECTORY_TYPE_INT:
			writeNetworkValue( output, m_int );
			break;
		case DIRECTORY_TYPE_FLOAT:
			writeNetworkValue( output, m_float );
			break;
		case DIRECTORY_TYPE_FIXED:
			writeNetworkValue( output, m_fixed );
			break;
		case DIRECTORY_TYPE_CHAR:
			output.write( (char *)&m_char, sizeof( m_char ) );
			break;
		case DIRECTORY_TYPE_BOOL:
			output.put( m_bool ? 1 : 0 );
			break;
