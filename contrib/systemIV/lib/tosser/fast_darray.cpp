...
FastDArray<T>::FastDArray()
	: DArray<T>(), numused( 0 ), firstfree( -1 )
{
}
...
FastDArray<T>::FastDArray( int newstepsize )
	: DArray<T>( newstepsize ), numused( 0 ), firstfree( -1 )
{
}
...
FastDArray<T>::FastDArray( FastDArray &&other ) noexcept
	: DArray<T>( std::move( other ) ), numused( other.numused ), freelist( std::move( other.freelist ) ), firstfree( other.firstfree )
{
	other.numused = 0;
	other.firstfree = -1;
}
