...
{
    // When converting a double to a signed 32 bit integer, there is the
    // question of what to do when the integer part of the double exceeds
    // the maximum signed 32 integer. The result on x86_64 is rather weird,
    // you get the minimum signed 32 bit integer! On arm64, you get the
    // maximum signed 32 bit integer, which makes more sense, but unfortunately
    // is a difference in behaviour that we can't have for deterministic maths
    // relied apon by DEFCON.
    //
    // Fortunately the underflow behaviour on both arm64 and x86 is the same.
    // That is, both return the minimum signed integer.
    
    if( m_value >= double(std::numeric_limits<int32_t>::max()) + 1.0 )
    {
        return double(std::numeric_limits<int32_t>::min());
    }
