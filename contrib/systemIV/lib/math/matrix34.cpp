...
void Matrix34::OrientRU( Vector3<float> const &_r, Vector3<float> const &_u )
{
	Vector3<float> tr = _r;
	tr.Normalise();
	f = tr ^ _u;
...
void Matrix34::OrientRF( Vector3<float> const &_r, Vector3<float> const &_f )
{
	Vector3<float> tr = _r;
	tr.Normalise();
	u = _f ^ tr;
...
void Matrix34::OrientUF( Vector3<float> const &_u, Vector3<float> const &_f )
{
	Vector3<float> tu = _u;
	tu.Normalise();
	r = tu ^ _f;
...
void Matrix34::OrientUR( Vector3<float> const &_u, Vector3<float> const &_r )
{
	Vector3<float> tu = _u;
	tu.Normalise();
	f = _r ^ tu;
...
void Matrix34::OrientFR( Vector3<float> const &_f, Vector3<float> const &_r )
{
	Vector3<float> tf = _f;
	tf.Normalise();
	u = tf ^ _r;
...
void Matrix34::OrientFU( Vector3<float> const &_f, Vector3<float> const &_u )
{
	Vector3<float> tf = _f;
	tf.Normalise();
	r = _u ^ tf;
...
Matrix34 const &Matrix34::RotateAround( Vector3<float> const &_rotationAxis )
{
...
Matrix34 const &Matrix34::FastRotateAround( Vector3<float> const &_norm, float _angle )
{
...
Matrix34 const &Matrix34::RotateAround( Vector3<float> const &_onorm, float _angle )
{
...
Vector3<float> Matrix34::DecomposeToYDR() const
{
	// x = dive, y = yaw, z = roll
	float x, y, z;
...
void Matrix34::Test()
{
	Vector3<float> g_upVector = Vector3<float>::UpVector();
	Vector3<float> g_zeroVector = Vector3<float>::ZeroVector();

	Matrix34 a( Vector3<float>( 0, 0, 1 ), g_upVector, g_zeroVector );
	Matrix34 b( Vector3<float>( 0, 0, 1 ), g_upVector, g_zeroVector );
	Matrix34 c = a * b;
	AppDebugOut( "c = a * b\n" );
	c.WriteToDebugStream();

	Vector3<float> front( 10, 20, 2 );
	front.Normalise();
	Vector3<float> up( 0, 1, 0 );
	Vector3<float> right = up ^ front;
	right.Normalise();
	up = front ^ right;
	up.Normalise();
	Matrix34 d( front, up, Vector3<float>( -1, 2, -3 ) );
	AppDebugOut( "d = \n" );
