...
{
	if ( !_archiveList || _archiveList->Size() == 0 )
		return;
...
BinaryReader *FileSystem::GetBinaryReader( const char *_filename )
{
	...
	if ( !reader )
	{
		...
		if ( mmfile )
			reader = new BinaryDataReader( mmfile->m_data, mmfile->m_size, _filename );
	}
#endif // NO_UNRAR
...
LList<char *> *FileSystem::ListArchive( const char *_dir, const char *_filter, bool fullFilename )
{
	...
	if ( m_archiveFiles.Size() > 0 )
	{
		...
		{
			if ( !unfilteredResults->ValidIndex( i ) )
				continue;
			...
			if ( stricmp( dirPart, _dir ) == 0 )
			{
				...
				{
					if ( !fullname.empty() )
						OrderedInsert( results, filePart );
					else
						OrderedInsert( results, fullname.c_str() );
				}
			}
		}
