...
NetRetCode NetSocketListener::StartListening(NetCallBack functionPointer, ThreadFunctions *threadFunctions )
{
    ...
    while( true )
    {
        ...
        if( threadFunctions && threadFunctions->StopRequested() ) break;
...
void NetSocketListener::StopListening( ThreadController *threadToStop )
{
	m_listening = false;
	m_binding = false;
	NetSleep(250);
    ...
    if( threadToStop ) threadToStop->Stop();
}
