...
static ThreadController                 s_authThread;
...
void Authentication_LoadKey( char *_key, const char *_filename )
{
    ...
    if( file )
    {
        int itemsRead = fscanf( file, "%s", _key );
        if( itemsRead == 1 ) success = true;
        fclose(file);
    }
...
void Authentication_SetKey( char *_key )
{
    ...
    // Trim end whitespace
    int keyLen = (int) strlen(s_authKey);
...
static void AuthenticationThread(ThreadFunctions *threadFunctions)
{
    ...
    while( !threadFunctions->StopRequested() )
    {
        ...
        threadFunctions->Wait( PERIOD_AUTHENTICATION_RETRY );
    }
...
void Authentication_RequestStatus( char *_key, int _keyId, char *_ip )
{
    ...
    if( !s_authThread.IsRunning() )
    {
        s_authThread.Start( AuthenticationThread );
    }
}
