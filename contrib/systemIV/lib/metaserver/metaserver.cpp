...
static ThreadController s_registeringOverWAN;
static ThreadController s_registeringOverLAN;
...
void ReceiveMetaServerData( Directory const &_directory )
{
	...
	target->m_data.reset( new Directory( _directory ) );
	int ttl = _directory.GetDataInt( NET_METASERVER_DATATIMETOLIVE );
	if ( ttl < 0 )
		target->m_timeOfExpire = -1;
	else
		target->m_timeOfExpire = GetHighResTime() + ttl;
...
void ReceiveDirectory( Directory const &_directory )
{
	...
	else if ( strcmp( cmd, NET_METASERVER_REGISTER ) == 0 )
	{
		...
		Directory dirCopy( _directory );

		dirCopy.CreateData( NET_METASERVER_IP, fromIP );
		dirCopy.CreateData( NET_METASERVER_PORT, localPort );

		MetaServer_UpdateServerList( fromIP, localPort, false, dirCopy );
	}
...
static void MetaserverListenCallback( NetUdpPacket const &udpData )
{
	// **** addresstostr
...
static NetCallBackRetType MetaserverListenThread( void *ignored )
{
...
void MetaServer_Connect( const char *_metaServerIp, int _metaserverPort, int _listenPort )
{
	...
	NetStartThread( MetaserverListenThread );
...
static void ServerRegisterWANThread( ThreadFunctions *thread )
{
#ifdef WAN_PLAY_ENABLED
	AppDebugOut( "MetaServer : WAN registration enabled\n" );

	while ( !thread->StopRequested() && s_connectedToWAN )
	{
		bool success = true;
...
{
#ifdef WAN_PLAY_ENABLED
	if ( s_connectedToWAN && !s_registeringOverWAN.IsRunning() )
	{
		s_registeringOverWAN.Start( ServerRegisterWANThread );
	}
#else
	AppDebugOut( "MetaServer WARNING : WAN Play is disabled\n" );
#endif
...
static void ServerRegisterLANThread( ThreadFunctions *thread )
{
#ifdef LAN_PLAY_ENABLED
	AppDebugOut( "MetaServer : LAN registration enabled\n" );

	while ( !thread->StopRequested() && s_socketToLAN )
	{
		bool success = true;
		...
		thread->Wait( PERIOD_SERVER_REGISTER_LAN );
	}
...
{
#ifdef LAN_PLAY_ENABLED
	if ( !s_registeringOverLAN.IsRunning() && s_socketListener )
	{
		//
...
{
	return ( s_connectedToWAN && s_registeringOverWAN.IsRunning() );
}


bool MetaServer_IsRegisteringOverLAN()
{
	return s_registeringOverLAN.IsRunning();
}
...
void MetaServer_UpdateServerList( char *_ip, int _port, bool _lanOrWan, Directory const &_server )
{
	...
	for ( int i = 0; i < s_serverList.Size(); ++i )
	{
		...
		if ( strcmp( thisServer->m_identity, serverIdentity ) == 0 ||
			 authKeyMatch )
		{
			thisServer->m_data.reset( new Directory( _server ) );
			thisServer->m_updateTime = GetHighResTime();
			if ( _lanOrWan == 0 )
				thisServer->m_receivedLan = true;
			if ( _lanOrWan == 1 )
				thisServer->m_receivedWan = true;
			s_serverListMutex.Unlock();
			return;
		}
	}
	...
	server->m_data.reset( new Directory( _server ) );
...
Directory *MetaServer_GetServer( char *_ip, int _port, bool _authRequired )
{
	...
	for ( int i = 0; i < s_serverList.Size(); ++i )
	{
		...
		{
			result = new Directory();
			result->CopyData( server->m_data.get() );
			break;
		}
	}
	...
	for ( int i = 0; i < s_serverList.Size(); ++i )
	{
			 ...
			 server->m_authResult == AuthenticationAccepted )
		{
			...
			if ( lanMatch || wanMatch )
			{
				...
				copyMe->CopyData( server->m_data.get() );
...
Directory *MetaServer_RequestData( const char *_dataType )
{
	//
	...
	for ( int i = 0; i < s_metaServerData.Size(); ++i )
	{
		...
		if ( stricmp( data->m_dataType, _dataType ) == 0 )
		{
			...
			if ( data->m_data.get() )
			{
				result = new Directory( *data->m_data.get() );
				data->m_retries = 0;
			}
...
void MetaServer_RemoveData( const char *_dataType )
{
	s_metaServerDataMutex.Lock();
