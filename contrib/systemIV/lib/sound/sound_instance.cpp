	...
	if ( m_adsrState != StateRelease )
	{
		...
		// Switch timer to appropriate clock
		{
			...
			if ( sdl2d && sdl2d->UsingPushMode() && g_preferences && g_preferences->GetInt( "SoundAudioClockedADSR", 1 ) )
			{
				unsigned freq = sdl2d->GetActualFreq();
				if ( freq > 0 )
					m_adsrTimer = (double)sdl2d->GetPlaybackSampleIndex() / (double)freq;
				else
					m_adsrTimer = GetHighResTime();
			}
			else
		...
		if ( m_loopDelayTimer > 0.0f )
		{
			...
			double now = GetHighResTime();
			{
				...
				if ( sdl2d && sdl2d->UsingPushMode() && g_preferences && g_preferences->GetInt( "SoundAudioClockedADSR", 1 ) )
				{
					unsigned freq = sdl2d->GetActualFreq();
					if ( freq > 0 )
						now = (double)sdl2d->GetPlaybackSampleIndex() / (double)freq;
				}
			}
			if ( now >= loopFinish )
			{
...
bool SoundInstance::UpdateParameter( SoundParameter &_param )
{
	...
	{
		const char *linkName = SoundParameter::GetLinkName( _param.m_link );
...
SoundObjectId SoundInstance::GetAttachedObject()
{
	...
	if ( !g_soundSystem->m_interface->DoesObjectExist( m_objId ) )
...
char *SoundInstance::GetDescriptor()
{
	...
	const char *positionType = GetPositionTypeName( m_positionType ); // m_positionType == TypeInEditor ? " editor" : "       ";
