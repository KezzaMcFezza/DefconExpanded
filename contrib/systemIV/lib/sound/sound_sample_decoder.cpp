...
static inline bool s_IsValidKernel( const SoundResampler::Kernel *k )
{
	...
	if ( k->taps > kMaxTaps || k->phases > kMaxPhases )
		return false;

	const size_t needed = (size_t)k->taps * (size_t)k->phases;
	if ( k->coeffs.size() < needed )
		return false;
	return true;
}
...
void SoundSampleDecoder::GetFrame( double _frame, bool _stereo, SoundResampler::Quality _quality, double _ratio, float &_outLeft, float &_outRight )
{
	...
	if ( cacheEndFrame < 0 )
		cacheEndFrame = 0;
	if ( cacheEndFrame > (int)totalFrames )
		cacheEndFrame = (int)totalFrames;
	...
	if ( cacheEndFrame > cacheStartFrame )
	{
		...
		if ( ensureEnd64 > (unsigned long long)m_numSamples )
			ensureEnd64 = (unsigned long long)m_numSamples;
...
{
	if ( !m_sampleCache )
		m_sampleCache = new signed short[m_numSamples];
	...
	switch ( m_fileType )
	{
		...
		case TypeWav:
			samplesRead = ReadWavData( &m_sampleCache[m_amountCached], _numSamples );
			break;
		case TypeOgg:
			samplesRead = ReadOggData( &m_sampleCache[m_amountCached], _numSamples );
			break;
	}
