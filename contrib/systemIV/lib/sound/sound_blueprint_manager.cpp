...
SoundEventBlueprint *SoundBlueprintManager::GetBlueprint( const char *_objectType )
{
...
{
	const char *_filename = "data/sounds.txt";
...
void SoundBlueprintManager::ParseSoundInstanceBlueprint( TextReader *_in, char *_objectName, SoundEventBlueprint *_source )
{
	...
	while ( stricmp( fieldName, "END" ) != 0 )
	{
		if ( stricmp( fieldName, "SOUNDNAME" ) == 0 )
		{
			...
			sib->m_instance->m_sourceType = atoi( _in->GetNextToken() );
		else if ( stricmp( fieldName, "POSITIONTYPE" ) == 0 )
			sib->m_instance->m_positionType = atoi( _in->GetNextToken() );
		else if ( stricmp( fieldName, "INSTANCETYPE" ) == 0 )
			sib->m_instance->m_instanceType = atoi( _in->GetNextToken() );
		else if ( stricmp( fieldName, "LOOPTYPE" ) == 0 )
			sib->m_instance->m_loopType = atoi( _in->GetNextToken() );
		else if ( stricmp( fieldName, "MINDISTANCE" ) == 0 )
			sib->m_instance->m_minDistance = atof( _in->GetNextToken() );
		else if ( stricmp( fieldName, "VOLUME" ) == 0 )
			sib->m_instance->m_volume.Read( _in );
		else if ( stricmp( fieldName, "FREQUENCY" ) == 0 )
			sib->m_instance->m_freq.Read( _in );
		else if ( stricmp( fieldName, "ATTACK" ) == 0 )
			sib->m_instance->m_attack.Read( _in );
		else if ( stricmp( fieldName, "SUSTAIN" ) == 0 )
			sib->m_instance->m_sustain.Read( _in );
		else if ( stricmp( fieldName, "RELEASE" ) == 0 )
			sib->m_instance->m_release.Read( _in );
		else if ( stricmp( fieldName, "LOOPDELAY" ) == 0 )
			sib->m_instance->m_loopDelay.Read( _in );
		else if ( stricmp( fieldName, "EFFECT" ) == 0 )
			ParseSoundEffect( _in, sib->m_instance );
		else
...
SampleGroup *SoundBlueprintManager::GetSampleGroup( const char *_name )
{
	for ( int i = 0; i < m_sampleGroups.Size(); ++i )
	{
		SampleGroup *group = m_sampleGroups[i];
...
SampleGroup *SoundBlueprintManager::NewSampleGroup( const char *_name )
{
...
bool SoundBlueprintManager::RenameSampleGroup( const char *_oldName, const char *_newName )
{
	//
