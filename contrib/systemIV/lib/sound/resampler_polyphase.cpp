...
QualityConfig GetQualityConfig( Quality quality )
{
	...
	{
		case Quality::Sinc64:
			return { quality, 64u, 256u };
		case Quality::Sinc96:
			return { quality, 96u, 256u };
		case Quality::Sinc128:
			return { quality, 128u, 512u };
		case Quality::Linear:
		default:
...
float BlackmanHarris( unsigned int n, unsigned int taps )
{
	...
	return a0 - a1 * cosf( theta ) + a2 * cosf( 2.0f * theta ) - a3 * cosf( 3.0f * theta );
}
...
std::vector<KernelBank> &GetBanks( Quality quality )
{
	...
	{
		case Quality::Sinc64:
			return g_state.banksSinc64;
		case Quality::Sinc96:
			return g_state.banksSinc96;
		case Quality::Sinc128:
			return g_state.banksSinc128;
		case Quality::Linear:
		default:
...
Quality QualityFromString( const char *name, Quality fallback )
{
	...
	if ( stricmp( name, "linear" ) == 0 )
		return Quality::Linear;
	if ( stricmp( name, "sinc64" ) == 0 )
		return Quality::Sinc64;
	if ( stricmp( name, "sinc96" ) == 0 )
		return Quality::Sinc96;
	if ( stricmp( name, "sinc128" ) == 0 )
		return Quality::Sinc128;
