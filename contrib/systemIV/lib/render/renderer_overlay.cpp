...
RendererOverlay::RendererOverlay()
	: m_showBufferStats( true ), m_showMemoryInfo( true ), m_showOpenGLInfo( true ), m_showFrameTiming( true ), m_showDetailedBuffers( true ), m_frameTimeHistory(), m_frameTimeIndex( 0 ), m_averageFrameTime( 0.0 )
{
...
void RendererOverlay::RenderVBOCacheStatistics( float &yPos )
{
	...
	auto renderVBOStat = [&]( const char *label, int vertices, int indices, size_t vertexSize )
	{
		...
		Colour vboColor = ( vertexPercent >= 40.0f ) ? red : ( vertexPercent >= 20.0f ) ? yellow
																						: grey;
...
int RendererOverlay::EstimateTextureSwitches()
{
	...
	if ( g_renderer2d->GetTextCalls() > 0 )
		switches++;
	if ( g_renderer2d->GetStaticSpriteCalls() > 0 )
		switches += 8;
	if ( g_renderer2d->GetRotatingSpriteCalls() > 0 )
		switches += 3;

	if ( g_renderer3d->GetTextCalls() > 0 )
		switches++;
	if ( g_renderer3d->GetStaticSpriteCalls() > 0 )
		switches += 8;
	if ( g_renderer3d->GetRotatingSpriteCalls() > 0 )
		switches += 3;
