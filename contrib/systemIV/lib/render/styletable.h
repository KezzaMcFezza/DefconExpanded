...
public:
    ...
    Style   *GetStyle           ( const char *_name );

    Colour  GetPrimaryColour    ( const char *_name );
    Colour  GetSecondaryColour  ( const char *_name );

    void    Clear               ();
    bool    Load                ( const char *_filename );
    bool    Save                ( const char *_filename=NULL );
};
