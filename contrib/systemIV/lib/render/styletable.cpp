...
bool StyleTable::Load( const char *_filename )
{
	delete[] m_filename;
	...
	while ( in->ReadLine() )
	{
		...
		{
			if ( stricmp( param, "PRIMARYCOL" ) == 0 )
				style->m_primaryColour.LoadFromHex( in->GetNextToken() );
			if ( stricmp( param, "SECONDARYCOL" ) == 0 )
				style->m_secondaryColour.LoadFromHex( in->GetNextToken() );
			if ( stricmp( param, "FONT" ) == 0 )
				strcpy( style->m_fontName, in->GetNextToken() );
			if ( stricmp( param, "NEGATIVE" ) == 0 )
				style->m_negative = (bool)atoi( in->GetNextToken() );
			if ( stricmp( param, "HORIZONTAL" ) == 0 )
				style->m_horizontal = (bool)atoi( in->GetNextToken() );
			if ( stricmp( param, "UPPERCASE" ) == 0 )
				style->m_uppercase = (bool)atoi( in->GetNextToken() );
			if ( stricmp( param, "SIZE" ) == 0 )
				style->m_size = atof( in->GetNextToken() );
...
bool StyleTable::Save( const char *_filename )
{
...
Style *StyleTable::GetStyle( const char *_name )
{
	for ( int i = 0; i < m_styles.Size(); ++i )
	{
		Style *style = m_styles[i];
...
Colour StyleTable::GetPrimaryColour( const char *_name )
{
...
Colour StyleTable::GetSecondaryColour( const char *_name )
{
	Style *style = GetStyle( _name );
	if ( !style )
	{
