...
{    
    ...
    g_renderer2d->Line        ( realX, realY+1, realX+m_w, realY+1, borderA );
    g_renderer2d->Line        ( realX+1, realY, realX+1, realY+m_h, borderA );

    g_renderer2d->RectFill    ( realX, realY, m_w, m_h, background );
    g_renderer2d->Line        ( realX, realY, realX + m_w, realY, borderB );
    g_renderer2d->Line        ( realX, realY, realX, realY + m_h, borderB );
    ...
    if( m_value && *m_value == true )
    {
        ...
        g_renderer2d->Line( realX+inset, realY+inset, realX+m_w-inset, realY+m_h-inset, col );
        g_renderer2d->Line( realX+inset, realY+m_h-inset, realX+m_w-inset, realY+inset, col );
    }
}
