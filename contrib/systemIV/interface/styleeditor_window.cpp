...
public:
    ...
    void MouseUp()
    {
        ...
        if( g_styleTable->m_styles.ValidIndex(realIndex) )
        {
			...
			char title[256];
            strcpy( title, LANGUAGEPHRASE("dialog_style_colour_editor") );
...
public:
    ...
    void FileSelected( char *_filename )
    {
        ...
        {
            char caption[512];
            strcpy( caption, LANGUAGEPHRASE("dialog_style_failed_load_style") );
...
class SaveStylesButton : public InterfaceButton
{
    void MouseUp()
    {
        ...
        if( success )
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_style_style_saved") );
        }
        else
        {
            strcpy( caption, LANGUAGEPHRASE("dialog_style_failed_save_style") );
        }
		strcat( caption, "data/styles/" );
...
{    
    ...
    if( g_styleTable->m_filename ) strcpy( m_filename, g_styleTable->m_filename );
}
