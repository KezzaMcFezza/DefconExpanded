...
{
    if (password.empty()) {
        Log::Err() << "ERROR: Empty passwords are not allowed for RCON\n";
        return false;
    }
...
#ifdef DEBUG
    Log::Out() << "Server encryption initialized (key hash: " 
               << std::hex << (*(uint32_t*)key.data()) << ")\n";
#endif
...
{
    if (!initialized) {
        Log::Err() << "Encryption not initialized during decrypt\n";
        return false;
    }

    size_t minSize = sizeof(uint32_t) + ServerEncryptedPacket::NONCE_SIZE + ServerEncryptedPacket::TAG_SIZE;
    if (len < minSize) {
        Log::Err() << "Packet too small for decryption: " << len << " bytes (minimum: " << minSize << ")\n";
        return false;
    }
    ...
    if (magic != ServerEncryptedPacket::MAGIC) {
        #ifdef DEBUG
        Log::Err() << "Invalid magic number in packet: " << std::hex << magic << " (expected: " << ServerEncryptedPacket::MAGIC << ")\n";
        #endif
        return false;
    }
...
#ifdef DEBUG
    Log::Out() << "Decrypting packet: magic=" << std::hex << magic 
               << ", encrypted size=" << std::dec << encryptedLen << " bytes\n";
#endif
    ...
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx) {
        Log::Err() << "Failed to create OpenSSL cipher context\n";
        return false;
    }
    ...
    do
    {
        ...
        if (EVP_DecryptFinal_ex(ctx, output.data() + outlen, &finalLen) != 1) {
            #ifdef DEBUG
            Log::Err() << "EVP_DecryptFinal_ex failed - authentication tag mismatch?\n";
            #endif
            break;
        }
