...
def extract_version_from_cmake():
    ...
    try:
        ...
        if match:
            ...
            print(f"Found DEFCON_VERSION in CMakeLists.txt: {version}")
            return version
        else:
...
def find_html_files():
    defcon_dir = Path(__file__).parent.parent.parent / "website" / "defcon"
    
    if not defcon_dir.exists():
        print(f"ERROR: Defcon directory not found at {defcon_dir}")
        return []
    
    html_files = list(defcon_dir.glob("defcon_*.html"))
...
def verify_build_files_exist(version):
    ...
    expected_files = [
        f"defcon_{version}.js",
        f"defcon_{version}.wasm"
    ]
    
    missing_files = []
    for filename in expected_files:
        file_path = defcon_dir / filename
...
def update_script_tag(html_file, new_version):
    try:
        ...
        current_pattern = rf'src\s*=\s*"defcon_{re.escape(new_version)}\.js"'
        if re.search(current_pattern, content):
            print(f"✓ Script tag in {html_file.name} already points to defcon_{new_version}.js")
            return True
        
        pattern = r'(src\s*=\s*")defcon_[^"]*\.js(")'
        replacement = rf'\1defcon_{new_version}.js\2'
        
        new_content = re.sub(pattern, replacement, content)
        
        if new_content == content:
            print(f"WARNING: No script tag found or updated in {html_file.name}")
            print("Looking for patterns like: src=\"defcon_*.js\"")
            ...
            defcon_refs = re.findall(r'defcon_[^"]*\.js', content)
            if defcon_refs:
                print(f"Found defcon references: {defcon_refs}")
        ...
        print(f"✓ Updated script tag in {html_file.name} to use defcon_{new_version}.js")
        return True
...
def rename_html_file(html_file, new_version):
    try:
        new_name = f"defcon_{new_version}.html"
...
def main():
    print("DEFCON Vanilla Version Updater")
    print("=" * 50)
    
    new_version = extract_version_from_cmake()
    if not new_version:
        print("FAILED: Could not extract DEFCON_VERSION from CMakeLists.txt")
        sys.exit(1)
    ...
    if not html_files:
        print("FAILED: No defcon_*.html files found")
        print("Make sure you have at least one HTML file in website/defcon/")
        sys.exit(1)
    ...
    if success_count > 0:
        ...
        print(f"✓ Script tags now point to: defcon_{new_version}.js")
        print(f"✓ HTML files renamed to: defcon_{new_version}.html")
    else:
