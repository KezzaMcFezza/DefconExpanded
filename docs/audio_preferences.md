...
- The in-game Sound Options menu exposes separate `Sound FX Quality` and `Music Quality` drop-downs. `Normal` selects `linear`, `High` selects `sinc64`, and `Very High` selects `sinc128`. The same panel now includes a `Sound Channels` selector that writes the `SoundChannels` preference; pick `64` to reduce CPU use on slower systems or `128` for maximum voice count. The mixer automatically pairs `64` with `8` music channels and `128` with `12`, keeping a dedicated music pool without wasting SFX voices.
