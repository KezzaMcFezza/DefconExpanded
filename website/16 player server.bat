...
start "Game Log Processor 8 Player Server" cmd /k python game_log_processor16player.py "game_events_16player.log" "game_logs"

:server_loop
set "game_start_time=%date:~10,4%-%date:~4,2%-%date:~7,2%T%time:~0,2%-%time:~3,2%-%time:~6,2%"
set "game_start_time=!game_start_time: =0!"

echo Starting Dedcon server for 16 player server...
dedcon16p.exe 16playerconfig.txt endgame8p.txt
