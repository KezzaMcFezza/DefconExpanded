...
def create_new_game_data():
    return {
        ...
        'duration': None,
        'gameType': '',
        'mapMod': None  # Add this field to store the map mod info
    }

def process_game_logs(game_events_file, output_directory):
    print(f"Starting game log processor for {game_events_file}")
...
def process_line(line, game_data, player_info, final_players, team_alliances, player_names, team_territories):
    if 'TEAM_ALLIANCE' in line:
        ...
        if match:
            ...
            # Check for UK mod in ServerMods setting
            if setting == 'ServerMods' and 'UK and Ireland' in value:
                game_data['mapMod'] = 'UK'
                print(f"Detected UK map mod: {value}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python LogProcessorUKVanilla.py <game_events_file> <output_directory>")
        sys.exit(1)
