...
def find_html_files():
    demo_dir = Path(__file__).parent.parent / "website" / "demo_recordings"
    
    if not demo_dir.exists():
        print(f"ERROR: Demo recordings directory not found: {demo_dir}")
        return []
    
    html_files = list(demo_dir.glob("replay_viewer_*.html"))
...
def main():
    ...
    if not html_files:
        ...
        print("Make sure you have at least one HTML file in website/demo_recordings/")
        sys.exit(1)
