...
def process_game_logs(server_output_file, game_events_file, output_directory):
    print(f"Starting game log processor for 1v1")
...
def process_player_scores(input_file, output_directory, game_data):
    ...
    if final_players:
        game_data['gameType'] = 'DefconExpanded | 1v1 | Totally Random' if len(final_players) > 2 else 'DefconExpanded | 1v1 | Totally Random'
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python game_log_processor1v1.py <server_output_file> <game_events_file> <output_directory>")
        sys.exit(1)
